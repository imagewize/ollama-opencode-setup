...
ollama create mistral-small3.2:24b-32k -f modelfiles/mistral-small3.2-24b-32k.Modelfile  # M4 24GB (recommended, no GPU tuning)
ollama create qwen3-coder:30b-32k -f modelfiles/qwen3-coder-30b-32k.Modelfile           # M4 24GB (coding MoE, needs raised GPU limit)
```

`ministral-3:8b-32k` is the recommended variant on M1 16GB — tested 100% GPU (11 GB footprint). 64k causes 27% CPU spillover on M1 16GB. On the Mac Mini M4 24GB, `mistral-small3.2:24b-32k` runs 100% GPU (19 GB) at the default ceiling and is the recommended no-tuning variant; its 64k variant spills 22% to CPU (25 GB) even with the raised GPU limit, so stick to 32k. `qwen3-coder:30b-32k` (21 GB) spills ~19% to CPU at the default ceiling and only reaches 98% GPU after `sudo sysctl -w iogpu.wired_limit_mb=21504` + Ollama restart — prefer it only when you want the coding MoE specifically. See [`modelfiles/README.md`](modelfiles/README.md) for the full GPU test results.
...
- **Recommended for Open Code (no GPU tuning)** → `mistral-small3.2:24b-32k` (19 GB, 32k ctx, 100% GPU, tool use confirmed 2026-06-30). The best-behaved 24B-class model that runs entirely on GPU at the default ceiling. The 64k variant spills 22% to CPU (25 GB) — stay at 32k. Keep tool-schema path descriptions neutral: the base model refuses with prose when a schema demands an "absolute path"
- **Coding MoE, but needs raised GPU limit** → `qwen3-coder:30b-32k` (21 GB, 32k ctx, coding-optimized MoE, ~34.5 tok/s warm, tool use confirmed). The base `qwen3-coder:30b` has no `num_ctx` baked in, so it runs at Ollama's 4k default inside Open Code — use the `:30b-32k` variant (`modelfiles/qwen3-coder-30b-32k.Modelfile`). Even then the 18 GB of weights spill ~19% to CPU at the default ceiling; only reaches 98% GPU after `sudo sysctl -w iogpu.wired_limit_mb=21504` + Ollama restart (tested 2026-06-30). Prefer the no-tuning options above unless you specifically want the coding MoE
